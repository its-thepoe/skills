...
name: family-taste
description: >-
  Family Values UI philosophy: Simplicity (gradual revelation), Fluidity
  (spatial motion, no teleports), Delight (by feature frequency). Use when
  designing or critiquing product UI, flows, sheets/trays, empty states,
  live numbers/charts, or “dead” interfaces; when the user says Family values,
  taste, progressive disclosure, torph, or liveline. Not for Framer Code/Plugin
  SDK (use framer-code-components-overrides or framer-plugins) or WCAG-only
  audits (use accessibility-review). Pairs with design-engineering for easing/duration specifics.
argument-hint: "[screen-or-flow]"
---

# Family taste (Family Values)

**Source:** [cristicretu/family-taste-skill](https://github.com/cristicretu/family-taste-skill) (`design-with-taste`). Philosophy from [Family](https://family.co) and [benji.org/family-values](https://benji.org/family-values).

This skill encodes the design philosophy behind Family — widely praised for feeling *alive*, *welcoming*, and *intentional*. Originally documented by Benji Taylor at [benji.org/family-values](https://benji.org/family-values).
...
The full skill is **longer than Lovable’s 10k Knowledge limit.** Use the condensed pack: [`LOVABLE.md`](LOVABLE.md), or sync this repo to GitHub and point root **`AGENTS.md`** at `family-taste/SKILL.md` for the complete version.
