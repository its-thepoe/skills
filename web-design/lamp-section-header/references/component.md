# Lamp Section Header Component

This reference captures the lamp-style section header pattern from the demo.

## Props

| Prop | Type | Default | Purpose |
| --- | --- | --- | --- |
| `children` | `React.ReactNode` | required | Headline or header content |
| `className` | `string` | - | Extra styles for the container |

## Implementation Notes

- Use Motion for the lamp cone and headline entrance.
- Build the lamp with mirrored `conic-gradient` beams, blur layers, and masking
  blocks to cut the light into a readable spotlight.
- Keep the headline inside a lower translated content layer so it sits in the
  glow, not on top of it.
- Use a dark container with a subtle border radius and controlled overflow.
- Keep the lamp color tunable from a single accent token.
- Clamp the headline width and size so the header stays short and balanced.
