...
name: framer-plugins
description: >-
  Framer Plugin SDK (framer-plugin): ManagedCollection, CMS sync modes,
  framer.showUI/notify, permissions, pluginData vs localStorage, Vite plugin.
  Use when building or debugging Framer plugins (npm create framer-plugin).
  Not for Framer site Code Components or Code Overrides on canvas — use
  framer-code-components-overrides instead.
argument-hint: "[plugin-topic]"
---

# Framer Plugin Development Guide

**Source:** [fredm00n/framerlabs](https://github.com/fredm00n/framerlabs) (MIT). **Scope:** Plugin SDK only — not canvas Code Components/Overrides.

**Official Framer docs (always check for API changes):** [references/framer-official-docs.md](references/framer-official-docs.md) — hub, reference, changelog, upgrading, CMS, permissions, and a shell recipe to re-download the framerlabs mirror.

You are an expert on the Framer Plugin SDK. Use this reference when building, debugging, or modifying Framer plugins. When the repo has `CLAUDE.md`, respect project-specific overrides there.

## Quick Reference

- **SDK package**: `framer-plugin` — pin to your Framer-supported version; check current release with `npm view framer-plugin version`
...
- **[framer-official-docs.md](references/framer-official-docs.md)** — Official Framer URLs + how to refresh vendored files from framerlabs
